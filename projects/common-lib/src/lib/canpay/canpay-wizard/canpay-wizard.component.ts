...
@Component({
  selector: 'canyalib-canpay-wizard',
...
export class CanpayWizardComponent implements OnInit {
  ...
  updateCurrentStep(step) {
    this.currStep = step;
    this.currentStep.emit(this.steps.find(s => s.value === step));
  }
  ...
  doStartPostAuthorisationProcess() {
    this.startPostAuthorisationProcess.emit(this.canPay());
  }
  ...
  canPay() {
    return {
      currStep: this.steps.find(step => step.value === this.currStep),
      amount: this.amount,
  ...
  doCancel() {
    this.cancel.emit(this.canPay());
  }

  finish() {
    this.updateCurrentStep(Step.completed);
    this.complete.emit(this.canPay());
  }
